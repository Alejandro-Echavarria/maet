...
<template>
    <div>
        ...
        <div class="space-y-6 max-w-5xl mx-auto">
            ...
            <span
                class="py-1 px-2 text-sm text-indigo-700 font-medium border-2 border-indigo-700 rounded-full hover:bg-indigo-700/5 transition ease-in-out duration-300">
                {{ job.category.name }}
            </span>
            </Link>

            <h1 :class="`text-center text-5xl lg:text-7xl py-1.5 md:text-6xl font-bold bg-gradient-to-r from-${job.color} via-gray-800/90 to-gray-800 bg-clip-text text-transparent animate-fade-in-up`"
                style="animation-delay: 0.36s;">
                {{ job.title }}
            </h1>

            <div class="space-y-4">
                <div class="flex justify-center flex-wrap items-center gap-2 animate-fade-in-up" style="animation-delay: 0.72s;">
                    <div v-if="job.link" class="flex flex-wrap items-center text-gray-600">
                        <a :href="job.link" target="_blank" rel="noreferrer noopener nofollow"
                            class="flex px-2" :title="job.title">
                            <span
                                class="text-sm py-1 px-2 text-indigo-700 border-2 border-indigo-700 font-medium rounded-full hover:bg-indigo-700/5 transition ease-in-out duration-300">
                                Visit the project
                            </span>
                        </a>
                    </div>
            ...
            <div class="flex justify-center flex-wrap gap-4 items-center text-gray-600 animate-fade-in" style="animation-delay: 1.1s;">
                <div v-for="(technology, index) in job.technologies" :key="'technology-' + technology.id">
                    ...
                    <div id="tech-container" class="flex gap-1 text-sm animate-fade-in-up"
