...
<template>
    <TransitionRoot appear :show="isShowing" enter="transition-opacity duration-75" enter-from="opacity-0"
        ...
        <TransitionChild class="space-y-2" enter="transition ease-in-out duration-300 transform"
            ...
            <span class="flex-1 break-keep">{{ item.label }}</span>
            </Link>
            <Disclosure v-else v-slot="{ open }" :default-open="hasActiveChild">
                <DisclosureButton :class="[
                    'flex w-full whitespace-nowrap text-left items-center py-2 px-3 text-sm font-semibold text-gray-700 rounded-lg dark:text-gray-200 hover:bg-indigo-200 dark:hover:bg-indigo-700 group transition duration-300 ease-linear',
